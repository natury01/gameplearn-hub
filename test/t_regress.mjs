...
console.log('\n═══ 3) ของเดิมในหน้าครูที่ต้องไม่หายไป ═══');
{
  ...
  const r = await p.evaluate(() => {
    ...
    return { roster: !!document.querySelector('[data-edit-stu]'),
      ...
      /* [V.1.6.19] เก็บข้อความบนแท็บมาด้วย — ใช้ยืนยันว่าเนื้อหาที่ถูกตัดยังหาเจอ */
      tabText: [...document.querySelectorAll('[data-tab]')].map((x) => x.textContent).join(' | ') };
  });
  ...
  /* [V.1.6.19 · ครูสั่ง] ตัดแท็บ "การคิดขั้นสูง (รายเกม)" ออก — เหลือ 2 แท็บ
     ข้อนี้เข้มกว่าเดิม ไม่ใช่ผ่อน: นอกจากนับแท็บแล้ว ยังยืนยันว่าเนื้อหาที่ถูกตัด
     **ไม่ได้หายไปจากระบบ** แค่ย้ายไปอยู่ใต้แท็บสมรรถนะซึ่งครอบทั้ง 6 ด้าน
     ถ้าวันหน้าใครเผลอตัดจนหาการคิดขั้นสูงไม่เจอเลย ข้อนี้จะแดง */
  /* [V.1.6.22] เลิกนับจำนวนแท็บ — นับเลขจับไม่ได้ว่า "แท็บผิดตัว" ถูกเพิ่มเข้ามา
     ตรวจ **ชุดแท็บจริง** แทน ซึ่งเข้มกว่า และบอกได้ว่าอะไรหาย/อะไรเกิน
     แท็บที่ควรมี: ผลสัมฤทธิ์ · สมรรถนะหลัก · เทียบเกณฑ์ (ครูสั่งเพิ่มใน V.1.6.22)
     แท็บที่ต้องไม่มี: การคิดขั้นสูงแยกเดี่ยว (ครูสั่งตัดใน V.1.6.19) */
  ok('ชุดแท็บถูกต้องตามที่ครูเคาะ (ผลสัมฤทธิ์ · สมรรถนะหลัก · เทียบเกณฑ์)',
    /ผลสัมฤทธิ์/.test(r.tabText || '') && /สมรรถนะหลัก/.test(r.tabText || '')
    && /เทียบเกณฑ์/.test(r.tabText || ''), r.tabText);
  ok('⭐ ไม่มีแท็บ "การคิดขั้นสูง (รายเกม)" แยกเดี่ยวอีกแล้ว (ครูสั่งตัด)',
    !/การคิดขั้นสูง \(รายเกม\)/.test(r.tabText || ''), r.tabText);
  ok('⭐ การคิดขั้นสูงไม่ได้หายไปจากระบบ — ย้ายไปอยู่ใต้แท็บสมรรถนะที่ครอบทั้ง 6 ด้าน',
    /สมรรถนะ/.test(r.tabText || ''), r.tabText);
