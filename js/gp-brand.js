...
(function () {
  ...
  function addThemeButton() {
    ...
    /* วางไว้หน้าปุ่มบัญชีตัวแรกที่เจอ — แต่ละหน้ามีปุ่มไม่เหมือนกัน
       ...
       ⚠️ `:scope >` สำคัญมาก ห้ามตัดทิ้ง — ต้องได้ "ลูกตรงของ .topbar" เท่านั้น
       ตั้งแต่ V.1.4.3 ปุ่ม "สร้างห้องเรียน" (.btn-primary) ย้ายเข้าไปอยู่ใน .navlinks
       ถ้าใช้ querySelector เปล่า ๆ จะไปเจอปุ่มที่อยู่ในเมนู แล้ว insertBefore โยน NotFoundError
       (เพราะโหนดนั้นไม่ใช่ลูกของ .topbar) → ปุ่มสลับสว่าง/มืดหายไปทั้งหน้า — เจอจริงมาแล้วรอบนี้ */
    var anchor = bar.querySelector(':scope > .btn-primary, :scope > #auth-home, '
      + ':scope > #auth-btn, :scope > #btn-logout, :scope > #logout');
  ...
  async function renderAuthButton() {
    ...
    /* ลิงก์เข้าห้องเรียน แทรกไว้ก่อนปุ่มออกจากระบบ ไม่งั้นครูจะกลับเข้าหน้าครูไม่ได้
       ...
       ⚠️ และยกเว้นหน้าที่วางปุ่ม "🏫 สร้างห้องเรียน" ไว้ในกลุ่มปุ่มฝั่งขวา (ลูกตรงของ .topbar)
       ฝั่งขวารับปุ่มได้ 2 ใบเท่านั้น — .wrap กว้างสูงสุด 1080px ต่อให้จอกว้างแค่ไหนก็เท่าเดิม
       ใส่ใบที่ 3 แล้วปุ่มจะตกบรรทัดทีละใบ ดูเหมือนหน้าเว็บพัง (วัดจริงแล้วที่ 1280 และ 1440)

       ตั้งแต่ V.1.4.3 ปุ่มนั้นย้ายเข้าไปอยู่ใน .navlinks (แทนที่ "เกมทั้งหมด" ตามที่ครูสั่ง)
       ซึ่งเป็นก้อนที่ยกลงแถวสองได้ทั้งก้อน ไม่แย่งที่กับปุ่มบัญชี → ลิงก์นี้จึงกลับมาได้
       จึงเช็กด้วย `.topbar > #new-room-btn` (ลูกตรง) ไม่ใช่ getElementById ทั้งหน้า
       ถ้าวันหนึ่งมีคนย้ายปุ่มกลับไปไว้ฝั่งขวา ตัวกันเบียดนี้จะกลับมาทำงานเองโดยไม่ต้องแก้โค้ด */
    if (!document.getElementById('auth-home') && !document.querySelector('.topbar > #new-room-btn')) {
