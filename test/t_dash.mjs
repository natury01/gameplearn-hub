...
console.log('\n═══ 3) ตัวเลขที่ฐานส่งมา ขึ้นจอครบและตรง ═══');
{
  ...
  ok('การกระจายผลสัมฤทธิ์ครบ 5 ช่วง',
    ['ดีเยี่ยม', 'ดี (70', 'พอใช้', 'ผ่าน', 'ยังไม่ถึงเกณฑ์'].every((x) => t.includes(x)), '');
...
console.log('\n═══ 4) สมรรถนะ 6 ด้าน — ด้านที่ยังไม่มีเกมวัด ต้องไม่ขึ้นเป็น 0 ═══');
{
  ...
  /* [V.1.6.33 · กฎ ก2] หน้าสาธารณะแยก "ระบบไม่ได้วัด" กับ "ยังไม่มีผลส่งมา" ไม่ได้
     ⇒ ถ้อยคำต้องจริงทั้งสองกรณี (TW เก็บหลักฐานอยู่แล้ว — คำเดิมโกหก) · ล็อกสองทาง */
  ok('⭐ ด้านที่ไม่มีผล เขียนว่า "ยังไม่มีผลสรุปด้านนี้ส่งขึ้นมา" ไม่ใช่เลข 0',
    st.txt.includes('ยังไม่มีผลสรุปด้านนี้ส่งขึ้นมา'), st.svgLabels.slice(0, 20));
  ok('คำเก่า "ยังไม่มีเกมวัดด้านนี้" หายไปจริง (ไม่เหลือสองสำนวน)',
    !st.txt.includes('ยังไม่มีเกมวัดด้านนี้'), '');
...
console.log('\n═══ 7) ดูเป็นตารางได้ (ข้อบังคับด้านการเข้าถึง) ═══');
{
  ...
  ok('ตารางสมรรถนะยังบอกว่าด้านไหนไม่มีผลสรุป', st.txt.includes('ยังไม่มีผลสรุปด้านนี้ส่งขึ้นมา'), '');
  await p.click('#tg-table');
  await sleep(500);
...
console.log('\n═══ 11) [V.1.6.40 · มติครู 8 ก.ย.] ค่าเฉลี่ยทุกด้านต้องพก "(จาก n คน)" ═══');
{
  ...
  ok('หัวเรื่องคะแนนเก็บเฉลี่ยบอกจำนวนรายการที่ใช้เฉลี่ย (1 คนต่อ 1 เกม)', /\(จาก \d+ รายการ\)/.test(st.main), '');
  await p.close();
}
